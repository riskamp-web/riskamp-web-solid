...
- **i18n — never call `t()` at module scope.** `t()` (and `currentLocale()`)
  reads a Solid store, so a module-body call runs at import — before any
  translation loads — and snapshots English forever; inside module-scope JSX it
